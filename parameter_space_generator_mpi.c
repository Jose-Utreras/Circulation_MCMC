...
double walker(char *name,double n1_min , double n1_max, double pc_min, double pc_max,
	...
	************************/
    ...
    for(i2=0;i2<steps;i2++){
        n1   = n1_min + (n1_max-n1_min)*i1/(1.0*steps-1);
  		pc   = pc_min*pow(r_factor,i2);
        temp = sigmas(n1, n1, 0.5*(pmin+pc), pmin, pc, Nbins, delta,dx);
    	fprintf(arx, "%f\t %f", n1, pc);
        for(i=0;i<Nbins;i++){
...
double L, out;
double pmin,pmax,dx,n1_min,n1_max,pc_min,pc_max;
char *name;
//clock_t start, end;
...
sscanf(argv[6],"%lf",&pc_min);
sscanf(argv[7],"%lf",&pc_max);
...
if(my_id==root_process){
	...
	out=walker(name,n1_min , n1_max, pc_min, pc_max,
